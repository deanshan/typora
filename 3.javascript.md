...
缺点：无法中间跳出循环（不支持==break==、==continue==操作，如若数组成员很多，将无法终止遍历来节省资源
...
循环遍历所有可枚举属性（包括对象自身的和原型上添加的和继承的）(不含Symbol属性)

能够遍历==对象==、==数组==、==字符串==，支持==break==、==continue==操作

缺点：会遍历原型上可枚举属性，需要通过==obj.hasOwnProperty(attr)==来过滤掉

### for-of

允许你遍历 Array（数组）, String（字符串）, Maps（映射）, Sets（集合）等可迭代的数据结构等
...
可以获取到对象实例的所有可枚举属性，其返回值为一个数组，数组元素为对象的==键名==

优点：它不会获取对象自身不可枚举属性和原型上的属性

缺点：因为它是es5的写法，所以只能遍历对象，非对象（如字符串）就会报错，但es6不会报错

### Object.getOwnPropertyNames()

获取==键名==数组，能够获取对象的不可枚举的属性
...
==获取Symbol属性名==，以上几种方法都无法遍历Symbol类型的对象，只能通过此方法来获取
...
+ 使用对象访问属性的时候，如果在本身的属性内找不到，就会去原型中查找。但是通过点语法去赋值的后，它不会优先去原型中查找
...
function Animal(){
    this.weight = 80;
    this.fn = function () { console.log('Animal')}
}
function Human(){
    this.weight = 60;
    this.fn = function () {console.log('Human')}
}
Human.prototype = new Animal();
Human.prototype.constructor = Human;
var h = new Human();
console.log(h.weight);//60
console.log(h.__proto__.weight);//80
h.fn();//Human
h.__proto__.fn();//Animal

// 去除属性屏蔽
//1、通过原型对象去找
// console.log(Human.prototype.weight);
// console.log(h.constructor.prototype.weight);

//2、删除本身的属性,不会删除原型中的属性。
// delete h["weight"]; //delete 只能删除自身的属性。
// console.log(h.weight);
  ```
